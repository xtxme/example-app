<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
        {{ __('Edit Social Media Link') }}</h2></x-slot>

    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6">
                <form method="POST" action="{{ route('social-links.update', $link) }}" class="space-y-4">
                    @csrf @method('PUT')
                    <div>
                        <label class="block text-sm font-medium mb-1">Platform Name</label>
                        <input name="platform" value="{{ old('platform', $link->platform) }}" required
                    ...
                    <div>
                        ...
                        <input name="url" type="url" value="{{ old('url', $link->url) }}" required
                               class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:text-gray-100"/>
